...
class CategoricalCrossEntropy(Loss):
    def __call__(self, predictions: NDArray, labels: NDArray) -> NDArray:
        epsilon = 1e-7
        # predictions: (num_classes, batch)
        # labels: (num_classes, batch) one-hot
        # compute mean cross-entropy over batch
        ce = -np.sum(labels * np.log(predictions + epsilon), axis=0)
        return np.mean(ce)

    def gradient(self, predictions: NDArray, labels: NDArray) -> NDArray:
        # gradient of CCE wrt logits when using softmax + CCE is (predictions - labels)/batch
        batch = labels.shape[1]
        return (predictions - labels) / batch
